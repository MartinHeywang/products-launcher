package com.martinheywang.products.launcher.view;
...
public class HomeView implements Initializable {
                ...
                try {
                    ...
                    try {
                        ...
                        if (isNewVersionAvailable(currentRelease, latestRelease)) {
                            ...
                            FileUtils.copyURLToFile(extractDownloadURLFromRelease(latestRelease, 0),
     ...
     */
    private URL extractDownloadURLFromRelease(JSONObject release, Integer assetIndex) {
        try {
            final String jsonValue = release.getJSONArray("assets").getJSONObject(assetIndex)
        ...
        try {
            if (new File(fileName).exists()) {
                prepareProcessBuilder(
                        "java -cp " + fileName + classpathSeparator + "plugins/* com.martinheywang.products.Launcher")
                                .start();
            } else {
                Platform.runLater(() -> {
                    info.setText(
                            "Le jeu n'a pas été trouvé sur votre machine. Si vous lancez le jeu pour la première fois, assurez d'avoir accès internet.");
                });
            }
        } catch (IOException e) {
            e.printStackTrace();
